...
export function Toaster({ position: defaultPosition = 'bottom-center' }: { position?: ToastPosition } = {}) {
    ...
    const groups = toasts.reduce<Partial<Record<ToastPosition, ToastItem[]>>>((acc, toast) => {
    ...
    return (
        <>
            {(Object.entries(groups) as [ToastPosition, ToastItem[]][]).map(([pos, items]) => (
                <Toast.Provider key={pos} swipeDirection="right">
                    {items.map(toast => (
                        <Toast.Root
                            key={toast.id}
                            className={cn(
                    ...
                    <Toast.Viewport className={cn('fixed flex flex-col gap-2 z-50 outline-none', positionClasses[pos])} />
                </Toast.Provider>
            ))}
        </>
    )
